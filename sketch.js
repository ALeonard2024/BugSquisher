...
let game = { score: 0, maxScore: 0, maxTime: 30, elapsedTime: 0, totalSprites: 10, state: GameState.Start, targetSprite: 2 };
...
function reset() {
  ...
  for(let i=0; i < game.totalSprites; i++) {
    animations[i] = new WalkingAnimation(random(spriteSheets),80,80,random(100,300),random(100,300),0,random(0.5,1),0,random([0,1]));
  }
}
...
class WalkingAnimation {
  ...
  draw() {
    ...
    if (this.vertical) {
      ...
      this.move(this.dy,this.sw / 2,height - this.sw / 2);
    }
    else {
      this.dx += this.moving*this.speed;
      this.move(this.dx,this.sw / 2,width - this.sw / 2);
    }
  }
